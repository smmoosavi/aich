import { effect, flush, state } from '../src';
...
describe('pin key', () => {
  ...
  test('duplicate pinKey usage throws error', async () => {
    effect(() => {
      pinKey('test', 'key1');
      pinKey('test', 'key1'); // Duplicate
    });
    expect(flush).toThrowErrorMatchingInlineSnapshot(
      `[Error: Duplicate test key. Key "key1" has already been used in this effect.]`,
    );
  });
});
